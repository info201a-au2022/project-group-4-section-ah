...
"),
  h3("Discussions"),
  p("
When it comes to collecting data for research purposes, some data sets found on the internet unless they are from 
governmental bureaus will have discrepancies, not enough data, or just skewed data. For variables such as age ranges, 
we were not sure if the same amount of subjects were tested for each age or age range group. When it came to the last 
visualization involving the depression and sleep data set. We were not outwardly told from the data description that the 
number of condition and controlled groups were equal when it came to the varying age range groups. There was also an implication 
with the fact that there was no control group for the age range of 55-59 for visualization three. This is indicating that the
experimenters did not conduct a fair experiment for all subjects to be expressed through the data."),

p("There was an implication in the stress and sleep dataset as well. There was not a very large data set. 
There were only 630 subjects that were examined for this experiment. In comparison to the other data sets in 
this project and other data sets on the web this is a fairly small sample size to work with and conduct data 
analysis on. Sample sizes are not supposed to emanate the entirety of the population, but their purpose is to be a small 
subsect of what we would assume the population would look like for this specific trail. However, this small sample size may
not do the best job of representing the overall sleeping patterns of the chosen population."),

p("There were implications in the screen time and sleep dataset as well. While this dataset did include a quite large sample size, 
one of more than 40,000 subjects, there were issues with the organization of data. This data set only included men and women as 
gender options in the variables section. This is limiting and not acknowledging intersectionality amongst our society. Everyone 
has their own unique experiences and when it comes to gender, discrimination with research such as this is widely known. This 
dataset was released in 2019, hence conversation about justice for more inclusive research methods, practices and experiments 
had been circulating around the web and social media. There are limitations and implications when grouping gender to be divided 
as male or female. It does not include those who are, for example, intersex. Many research datasets such as this one may refer to 
